...
export default function useTranscribe({
  ...
  const handleTranscribeFinish = useCallback(async (taskId) => {
    if (!taskId) return
    ...
    try {
      ...
      fetch(`${API_URL}/transcribe/summary/${taskId}`, { cache: "no-store" })
  ...
  }, [setStatus, setSentences, sentencesRef, timebaseRef, setOriginalSpkList, setHasSavedState, saveState, setSummary, numSpeakersRef, addLog])
  ...
  }, [setAudioPath, setSentences, setCurrentWordId, setSelectedWordIds, setFocusedWord, setCurrentTime, setIsPlayingState, sentencesRef, timelineIndexRef])
